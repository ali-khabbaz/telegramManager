...
if (cluster.isMaster) {
	...
	app.post('/app/consituency', function (req, res) {
		...
		showDb(search_person).then(function (result) {
			...
			var res_query = "select * from users where id = -1";
	...
	app.post('/app/advertise', function (req, res) {
		...
		var search_advertise =
			'select ad.day,ad.count,ad.content,ad.titel,ad.icon_url,p1.chanalLink,c.regionId ' +
			' from advertise ad left outer join  people p1 ' +
			' on ad.candidaId = p1.id ' +
			' join city c on (ad.regionId =c.regionId) join people p2 on ' +
			' ( p2.cityId=c.id and p2.telegramID = ' + req.body.telegramID + ')';
		var arr = [];
	...
	app.post('/app/advertise_insert', function (req, res) {
		...
		showDb(search_advertise).then(function (result) {
			if (result.length == 0) {
				var query = "INSERT INTO advertise (regionId,candidaId,day,count,content,titel,icon_url" +
					") VALUES ('" + req.query.regionId + "', " +
					"'" + req.query.candidaId + "', " + "'" + req.query.day + "', " + "'" + req.query.count + "', " + "'" + req.query.content + "', " +
					"'" + req.query.title + "', " + "'" + req.query.iconUrl + "' )";
