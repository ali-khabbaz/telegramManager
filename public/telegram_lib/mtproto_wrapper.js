...
.factory('MtpApiManager', function (Storage,
	MtpAuthorizer, MtpNetworkerFactory, MtpSingleInstanceService, AppRuntimeManager,
	ErrorService, qSync, $rootScope, $q, TelegramMeWebService) {
	...
	function mtpSetUserAuth(dcID, userAuth) {
		var fullUserAuth = angular.extend({
			dcID: dcID
		}, userAuth);
		Storage.set({
			dc: dcID,
	...
	function mtpLogOut() {
		...
		return Storage.get.apply(Storage, storageKeys).then(function (storageResult) {
			...
			for (var i = 0; i < storageResult.length; i++) {
				if (storageResult[i]) {
					logoutPromises.push(mtpInvokeApi('auth.logOut', {}, {
						dcID: i + 1
					}));
				}
			}
				...
				function (error) {
					...
					if (error.code == 401 && baseDcID == dcID) {
						...
						if (cachedExportPromise[dcID] === undefined) {
							...
							mtpInvokeApi('auth.exportAuthorization', {
								dc_id: dcID
							}, {
								noErrorBox: true
...
.factory('MtpApiFileManager', function ($rootScope,MtpApiManager, $q, qSync, FileManager, IdbFileStorage, TmpfsFileStorage, MemoryFileStorage, WebpManager) {
		...
		return cachedDownloadPromises[fileName] = fileStorage.getFile(fileName).then(function (blob) {
			...
			var downloadPromise = downloadRequest(location.dc_id, function () {
				...
				if (!inputLocation._ || inputLocation._ == 'fileLocation') {
					inputLocation = angular.extend({}, location, {
						_: 'inputFileLocation'
					});
				}
					...
					reader.onloadend = function (e) {
						...
						MtpApiManager.invokeApi(isBigFile ? 'upload.saveBigFilePart' : 'upload.saveFilePart', {
							...
							if (doneParts >= totalParts) {
								...
								resolved = true;
								console.log('-----------------0000',doneParts );
								setProgress(1);
							} else {
								console.log(dT(), 'Progress', doneParts * partSize / fileSize);
								deferred.notify({
									done: doneParts * partSize,
									total: fileSize
								});
								setProgress(doneParts * partSize / fileSize);
							}
						}, errorHandler);
					};
...
.service('MtpSingleInstanceService', function (_, $rootScope, $compile, $timeout,
	$interval, $modalStack, Storage, AppRuntimeManager, IdleManager, ErrorService, MtpNetworkerFactory) {
