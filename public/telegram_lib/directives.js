...
.directive('myDialogsList', function ($window, $timeout) {
	...
	function link($scope, element, attrs) {
		var dialogsWrap = $('.im_dialogs_wrap', element)[0],
			...
			panelWrapSelector = Config.Mobile && attrs.modal ? '.mobile_modal_body .im_dialogs_panel' : '.im_dialogs_panel',
...
.directive('myCountriesList', function ($window, $timeout) {
	...
	function link($scope, element, attrs) {
		...
		function updateSizes() {
			$(element).css({
				height: $($window).height() - (panelWrap && panelWrap.offsetHeight || 0) - (searchWrap && searchWrap.offsetHeight || 0) - (Config.Mobile ? 46 + 18 : 200)
			});
			$(countriesWrap).nanoScroller();
		}
...
.directive('mySessionsList', function ($window, $timeout) {
	...
	function link($scope, element, attrs) {
		...
		function updateSizes() {
			$(element).css({
				height: Math.min(760, $($window).height() - (Config.Mobile ? 46 + 18 : 200))
			});
			$(sessionsWrap).nanoScroller();
		}
...
.directive('myStickersList', function ($window, $timeout) {
	...
	function link($scope, element, attrs) {
		...
		function updateSizes() {
			$(element).css({
				height: Math.min(500, $($window).height() - (Config.Mobile ? 46 + 18 : 200))
			});
			$(stickersWrap).nanoScroller();
		}
...
.directive('myHistory', function ($window, $timeout, $rootScope, $transition) {
	...
	function link($scope, element, attrs) {
		...
		function updateBottomizer() {
			$(historyMessagesEl).css({
				marginTop: 0
			});
			var marginTop = scrollableWrap.offsetHeight - historyMessagesEl.offsetHeight - 20 - (Config.Mobile ? 0 : 39);
	...
	function link($scope, element, attrs) {
		...
		var composer = new MessageComposer(messageField, {
			...
			getPeerImage: function (element, peerID, noReplace) {
				console.log('-------getPeerImage---------', element, peerID, noReplace);
				...
				peerPhotoCompiled(scope, function (clonedElement) {
					console.log('-------peerPhotoCompiled---------', peerPhotoCompiled);
		...
		fileSelects.on('change', function () {
			...
			$scope.$apply(function () {
				...
				/*console.log('--########--fileSelects.on-----', self.files, Array.prototype.slice.call(self.files),
						$(self).hasClass('im_media_attach_input')); setTimeout(function () {
						try {
							self.value = '';
						} catch (e) {};
					}, 1000);*/
			...
			if (dragStarted == 2) {
				...
				if (e.type == 'dragenter' || e.type == 'dragover') {
					if (dragStateChanged) {
						...
						$(dropbox)
							.css({
								height: messageFieldWrap.offsetHeight + 2,
...
.directive('myLoadThumb', function (MtpApiFileManager, FileManager) {
	...
	function link($scope, element, attrs) {
		...
		var cachedBlob = MtpApiFileManager.getCachedFile(
			$scope.thumb &&
			$scope.thumb.location && !$scope.thumb.location.empty &&
		...
		var cleanup = attrs.watch ? angular.noop : function () {
			setTimeout(function () {
				$scope.$destroy()
				stopWatching();
			}, 0);
...
.directive('myLoadFullPhoto', function (MtpApiFileManager, FileManager, _) {
	...
	function link($scope, element, attrs) {
		...
		var imgElement = $('img', element)[0],
			...
			resize = function () {
				resizeElements.css({
					width: $scope.fullPhoto.width,
					height: $scope.fullPhoto.height
				});
		...
		$scope.$watchCollection('fullPhoto.location', function () {
			...
			apiPromise.then(function (blob) {
				...
				if (e && e.type == 'FS_BROWSER_UNSUPPORTED') {
					$scope.error = {
						html: _('error_browser_no_local_file_system_image_md', {
					...
					$scope.error = {
						text: _('error_image_download_failed'),
						error: e
					};
				}
			}, function (progress) {
...
.directive('myLoadVideo', function ($sce, AppVideoManager, ErrorService, _) {
	...
	function link($scope, element, attrs) {
		...
		downloadPromise.then(function () {
			...
			if (e && e.type == 'FS_BROWSER_UNSUPPORTED') {
				$scope.error = {
					html: _('error_browser_no_local_file_system_video_md', {
						'moz-link': '<a href="{0}" target="_blank">{1}</a>',
						'chrome-link': '<a href="{0}" target="_blank">{1}</a>',
						'telegram-link': '<a href="{0}" target="_blank">{1}</a>'
					})
				...
				$scope.error = {
					text: _('error_video_download_failed'),
					error: e
				};
			}
...
.directive('myLoadingDots', function ($interval) {
	...
	function link($scope, element, attrs) {
		element.html(isAnimationSupported(element[0]) ? '<div class="loading_dots"><i></i><i></i><i></i></div>' : '...');
	}
...
.directive('myModalWidth', function () {
	...
	function link($scope, element, attrs) {
		attrs.$observe('myModalWidth', function (newW) {
			$(element[0].parentNode.parentNode).css({
				width: parseInt(newW) + (Config.Mobile ? 0 : 32)
			});
		});
	};
...
.directive('myCustomBackground', function () {
	...
	function link($scope, element, attrs) {
		...
		$('html').css({
			background: attrs.myCustomBackground
		});
	...
	function link($scope, element, attrs) {
		...
		var update = function () {
			...
			if (onlineCount > 1 || onlineCount == 1 && !wasMe) {
				html = _('group_modal_participants', {
					total: html,
					online: onlinePluralize(onlineCount)
				});
			}
				...
				downloadPromise.then(function () {
					onContentLoaded(function () {
						...
						setTimeout(function () {
							...
							if ($scope.message && !$scope.message.out &&
...
.directive('myScrollToOn', function () {
	...
	.directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', '$interpolate', function ($q, $parse, $http, $sce, $timeout, $templateCache, $interpolate) {
		...
		$templateCache.put(TEMPLATE_URL,
			...
			'  <input id="{{id}}_value" required name="{{inputName}}" ng-class="{\'angucomplete-input-not-empty\': notEmpty}" ng-model="searchStr" ng-disabled="disableInput" type="{{inputType}}" placeholder="{{placeholder}}" maxlength="{{maxlength}}" ng-focus="onFocusHandler()" class="{{inputClass}}" ng-focus="resetHideResults()" ng-blur="hideResults($event)" autocapitalize="off" autocorrect="off" autocomplete="off" ng-change="inputChangeHandler(searchStr)"/>' +
