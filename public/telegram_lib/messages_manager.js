...
.service('AppMessagesManager', function ($q, $rootScope, $location, $filter, $timeout,
	$sce, ApiUpdatesManager, AppUsersManager, AppChatsManager, AppPeersManager,
	AppPhotosManager, AppVideoManager, AppDocsManager, AppStickersManager,
	AppAudioManager, AppWebPagesManager, MtpApiManager, MtpApiFileManager,
	RichTextProcessor, NotificationsManager, PeersSelectService, Storage,
	AppProfileManager, FileManager, TelegramMeWebService, ErrorService, StatusManager, _) {
	...
	function getDialogByPeerID(peerID) {
		...
		//console.log('-----getDialogByPeerID---', dialogsStorage , ' ' , peerID);
		return [];
	}
		...
		return MtpApiManager.invokeApi('messages.getDialogs', {
			...
			angular.forEach(dialogsResult.dialogs, function (dialog) {
				...
				if (historiesStorage[peerID] === undefined) {
					var historyStorage = {
						count: null,
						history: [dialog.top_message],
						pending: []
					};
					historiesStorage[peerID] = historyStorage;
					if (mergeReplyKeyboard(historyStorage, message)) {
						$rootScope.$broadcast('history_reply_markup', {
							peerID: peerID
						});
					}
				}
	...
	function mergeReplyKeyboard(historyStorage, message) {
		...
		if (!message.reply_markup && !message.out && !message.action) {
			return false;
		}
		...
		if (messageReplyMarkup) {
			...
			if (messageReplyMarkup.pFlags.selective && !(message.flags & 16)) {
				return false;
			}
		...
		if (message.out) {
			if (lastReplyMarkup) {
				if (lastReplyMarkup.pFlags.one_time && !lastReplyMarkup.pFlags.hidden &&
		...
		if (message.action &&
			...
			(lastReplyMarkup ? message.action.user_id == lastReplyMarkup.fromID : AppUsersManager.isBot(message.action.user_id))
		) {
	...
	function flushHistory(inputPeer) {
		...
		return MtpApiManager.invokeApi('messages.deleteHistory', {
			...
			$rootScope.$broadcast('dialog_flush', {
				peerID: peerID
			});
		});
	}
	...
	function sendText(peerID, text, options) {
		...
		$rootScope.$broadcast('history_append', {
			peerID: peerID,
			messageID: messageID,
			my: true
		});
	...
	function sendOther(peerID, inputMedia, options) {
		...
		case 'inputMediaContact':
			media = angular.extend({}, inputMedia, {
	...
	function wrapForHistory(msgID) {
		...
		if (message.media) {
			...
			case 'messageActionChannelEditTitle':
				message.action.rTitle = RichTextProcessor.wrapRichText(message.action.title, {
					noLinks: true,
					noLinebreaks: true
	...
	function regroupWrappedHistory(history, limit) {
		...
		for (i = start; i < end; i++) {
			...
			if (curMessage.views && !incrementedMessageViews[curMessage.mid]) {
			...
			if (prevMessage &&
				...
				!prevMessage.fwdFromID == !curMessage.fwdFromID && !prevMessage.action && !curMessage.action &&
	...
	$rootScope.$on('apiUpdate', function (e, update) {
		...
		case 'updateNewChannelMessage':
			...
			if (update._ == 'updateNewChannelMessage' && !(message.flags & 16 || message.flags & 2 || (message.flags & 256) == 0)) {
			...
			angular.forEach(historiesUpdated, function (updatedData, peerID) {
				...
				if (foundDialog) {
					if (updatedData.unread) {
						...
						$rootScope.$broadcast('dialog_unread', {
							peerID: peerID,
							count: foundDialog[0].unread_count
						});
					}
				}
		...
		case 'updateChannel':
			...
			if (hasDialog != needDialog) {
				if (needDialog) {
					...
					if (foundDialog[0]) {
						dialogsStorage.dialogs.splice(foundDialog[1], 1);
						$rootScope.$broadcast('dialog_drop', {
							peerID: peerID
						});
					}
				}
	...
	function setDialogType(argument) {
		typeofDialog = argument;
	}
	...
	function setAllMyDialogs(dialogs) {
		AllMyDialog = dialogs;
	}

	function getAllMyDialogs() {
		return AllMyDialog;

	}

	function setChanels(ch) {
		chanelList = ch;
	}
	...
	return {
		...
		setAllMyDialogs: setAllMyDialogs,
		getAllMyDialogs: getAllMyDialogs,
